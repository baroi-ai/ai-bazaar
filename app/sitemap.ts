...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  try {
    ...
    // 3. Define all your static utility and structural pages
    const staticPages = [
      {
    ...
    // 4. Combine static pages and dynamic apps together
    return [...staticPages, ...appUrls];

  } catch (error) {
    console.error("Error generating sitemap:", error);
    
    // Fallback basic list if database fails so the sitemap file doesn't crash entirely
    return [
      { url: 'https://aibazaars.store', lastModified: new Date() },
      { url: 'https://aibazaars.store/apps', lastModified: new Date() },
      { url: 'https://aibazaars.store/download', lastModified: new Date() },
      { url: 'https://aibazaars.store/pricing', lastModified: new Date() },
    ];
  }
}