        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <aside className="lg:col-span-4 flex flex-col items-center bg-zinc-900/40 border border-zinc-800/80 p-6 md:p-8 rounded-3xl lg:sticky lg:top-24 shadow-2xl">
            ...
            <div className="w-full flex flex-col gap-3">
              ...
              {/* Only show Divider if we have Daemon buttons AND regular download links below it */}
              {(isDaemonApp && (hasAnyDownloads || hasExternalLinks)) && (
              ...
              {hasPlaystore && (
                <a href={downloadLinks.playstore} target="_blank" rel="noopener noreferrer" className="w-full flex items-center gap-4 bg-black border border-zinc-700 hover:border-zinc-500 rounded-xl px-4 py-3 transition-all group">
                  <svg className="w-8 h-8 text-emerald-500 fill-current group-hover:scale-105 transition-transform shrink-0" viewBox="0 0 24 24" aria-hidden="true"><path d="M22.018 13.298l-3.919 2.218-3.515-3.493 3.543-3.521 3.891 2.202a1.49 1.49 0 0 1 0 2.594zM1.026 1.106a1.48 1.48 0 0 0-.426 1.054v19.676a1.485 1.485 0 0 0 .445 1.058l11.458-11.385L1.026 1.106zM13.511 12.519L2.247 23.684l10.155-5.746 2.122-2.118-1.013-3.301zM2.21 .316L13.511 11.48l1.042-3.327L2.21.316z"/></svg>
                  <div className="flex flex-col text-left">
              ...
              {/* DIVIDER */}
              {(hasAnyDownloads && hasExternalLinks && !isDaemonApp) && (
                <hr className="border-zinc-800/80 my-2 w-full" />
              )}
              ...
              {hasGithub && (
                <a href={appData.github_url} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-lg bg-zinc-800/50 hover:bg-zinc-800 text-zinc-300 text-sm font-medium transition-colors border border-zinc-700/50">
                  <svg className="w-4 h-4 fill-current shrink-0" viewBox="0 0 24 24" aria-hidden="true"><path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.166 6.839 9.489.5.092.682-.217.682-.483 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.068.069-.068 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.579.688.481C19.137 20.162 22 16.418 22 12c0-5.523-4.477-10-10-10z"/></svg>
                  View Source Code
                </a>
              )}
            </div>
