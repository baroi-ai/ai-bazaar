...
const CompareSlider = ({ original, enhanced }: { original: string; enhanced: string }) => {
  ...
  const isRemoteOriginal = original.startsWith("http") || original.startsWith("blob");
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 w-full h-full">
        <Image src={enhanced} alt="Enhanced" fill className="object-contain" priority unoptimized={true} />
      </div>
      ...
      >
        <div className="absolute top-0 left-0 h-full" style={{ width: containerWidth ? `${containerWidth}px` : "100%" }}>
          {isRemoteOriginal ? (
            <Image src={original} alt="Original" fill className="object-contain" priority unoptimized={true} />
          ) : (
...
export default function ImageUpscalerPage() {
  ...
  return (
    <div className="flex flex-col bg-[#050508] text-gray-300 font-sans scroll-smooth">
      ...
      <div className="flex flex-col min-h-screen">
        ...
        <div className="w-full px-4 pb-8 pt-2 border-t border-zinc-900/40">
          <div className="max-w-4xl mx-auto space-y-3.5">
            ...
            <div className="flex items-center justify-center gap-2 md:gap-3 w-full">
              ...
              <Button variant="outline" size="icon" onClick={() => setIsSettingsOpen(true)} disabled={isLoading} className="h-10 w-10 shrink-0 border-zinc-800 bg-[#0e0e0e] text-zinc-400 hover:text-white rounded-xl active:scale-95">
      ...
      <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
        <DialogContent className="bg-[#0e0e0e] border-zinc-800 text-zinc-200 sm:max-w-md rounded-2xl p-5">
          <DialogHeader>
            <DialogTitle className="text-white text-lg font-bold">Advanced Settings</DialogTitle>
            <DialogDescription className="text-zinc-400 text-xs">Fine-tune your upscaling parameters.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-3">
            <div className="grid gap-2">
              <Label htmlFor="negative-prompt" className="text-zinc-300 text-xs">Negative Prompt</Label>
              <Textarea id="negative-prompt" value={negativePrompt} onChange={(e) => setNegativePrompt(e.target.value)} placeholder="blurry, distorted, low quality" className="bg-[#0a0a0a] border-zinc-800 text-white rounded-xl focus:border-sky-500 text-xs" rows={3} />
            </div>
          </div>
          <DialogFooter>
            <DialogClose asChild>
              <Button type="button" className="bg-sky-600 hover:bg-sky-500 text-white rounded-xl font-bold w-full text-xs h-10">Save Changes</Button>
            </DialogClose>
          </DialogFooter>
        </DialogContent>
      </Dialog>
