...
export default function PDFToWordPage() {
  ...
  const [outputName, setOutputName] = useState<string>("document-data");
  ...
  // --- Extract Text and Compile Word Document ---
  const handleConvertToWord = async () => {
    if (!fileBuffer || !sourceFile || !pdfjsLoaded) return;

    setIsProcessing(true);
    toast.info("Parsing PDF content and building Word structure...");
    ...
    try {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Join page string tokens
        const pageText = textContent.items
          .map((item: any) => item.str)
          .join(" ");

        paragraphsList.push(`--- Page ${i} ---`);
        paragraphsList.push(pageText);
      }

      // Compile Genuine DOCX document nodes using docx package
      const wordDoc = new Document({
        sections: [
          {
            properties: {},
            children: paragraphsList.map(
              (pText) =>
                new Paragraph({
                  children: [new TextRun({ text: pText, size: 24 })], // size 24 = 12pt
                })
            ),
          },
        ],
      ...
      link.href = url;
      link.download = outputName.trim().endsWith(".docx")
        ? outputName.trim()
        : `${outputName.trim()}.docx`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);

      toast.success("Word DOCX document compiled successfully!");
    } catch (err) {
      console.error("PDF to Word conversion failed:", err);
      toast.error("Failed to parse and export PDF to Word.");
    } finally {
          ...
          <div className="lg:col-span-7 bg-[#0e0e0e] border border-zinc-800/80 rounded-2xl md:rounded-[2rem] flex flex-col relative overflow-hidden shadow-2xl p-4 md:p-6 justify-center items-center">
            ...
            {!sourceFile ? (
              ...
              >
                ...
                <h1 className="text-2xl md:text-3xl font-bold mb-3 text-white">Convert PDF to Word</h1>
                <p className="text-zinc-400 max-w-md mb-6 text-sm leading-relaxed px-4">
                  Drag and drop your PDF here, or click to browse. Build genuine editable Microsoft Word files locally.
                </p>
                <div className="flex flex-wrap justify-center gap-3">
                  <div className="flex items-center gap-2 px-3.5 py-1.5 bg-zinc-900 border border-zinc-800 text-zinc-400 rounded-full text-xs font-medium">
                    <Cpu className="w-3.5 h-3.5 text-cyan-400" /> Private Local Conversion
                  </div>
                  <div className="flex items-center gap-2 px-3.5 py-1.5 bg-zinc-900 border border-zinc-800 text-zinc-400 rounded-full text-xs font-medium">
                    <Layers className="w-3.5 h-3.5 text-emerald-400" /> genuine DOCX packages
                  </div>
                </div>
              </div>
            ) : (
              ...
              {!sourceFile ? (
                ...
                >
                  {isProcessing ? (
                    ...
                    <>
                      <FileText className="w-4 h-4 mr-2" /> Convert PDF to Word
                    </>
                  )}
                </Button>
              )}
          ...
          <section className="p-6 md:p-10 rounded-[2rem] border border-zinc-800/50 bg-[#0e0e0e]/50 backdrop-blur-xl relative overflow-hidden shadow-2xl">
            ...
            <div className="text-center mb-8 relative z-10">
              <h2 className="text-2xl md:text-3xl font-bold text-white mb-3">
                Local Client-Side <span className="text-cyan-400">PDF to Word Converter</span>
              </h2>
              <p className="text-zinc-400 text-xs md:text-sm max-w-xl mx-auto leading-relaxed">
                Convert your PDF files to editable Microsoft Word (.docx) documents. Processes text layers and maps them directly to paragraph structures offline.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 relative z-10">
              <div className="bg-[#0a0a0a]/60 border border-zinc-800/80 p-5 rounded-2xl hover:border-cyan-500/20 transition">
                ...
                <h3 className="text-sm md:text-base font-bold text-white mb-2">Editable Word Content</h3>
                <p className="text-zinc-400 text-xs leading-relaxed">
                  Extracts text items from the PDF layout structures, compiling them into a fully selectable text flow for Microsoft Word edits.
                </p>
              </div>

              <div className="bg-[#0a0a0a]/60 border border-zinc-800/80 p-5 rounded-2xl hover:border-cyan-500/20 transition">
                <div className="w-10 h-10 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center justify-center text-lg mb-4">🏎️</div>
                <h3 className="text-sm md:text-base font-bold text-white mb-2">genuine DOCX Packages</h3>
                <p className="text-zinc-400 text-xs leading-relaxed">
                  Writes output binary elements using OpenXML standard specifications, producing authentic Word documents (.docx) natively.
                </p>
              </div>

              <div className="bg-[#0a0a0a]/60 border border-zinc-800/80 p-5 rounded-2xl hover:border-cyan-500/20 transition">
                <div className="w-10 h-10 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center justify-center text-lg mb-4">🔀</div>
                <h3 className="text-sm md:text-base font-bold text-white mb-2">Page-Break Segmenting</h3>
                <p className="text-zinc-400 text-xs leading-relaxed">
                  Inserts page demarcation lines between different pages parsed from the source PDF, keeping the structure aligned.
                </p>
              </div>
              ...
              <div className="bg-[#0a0a0a]/60 border border-zinc-800/80 p-5 rounded-2xl hover:border-cyan-500/20 transition">
                ...
                <p className="text-zinc-400 text-xs leading-relaxed">
                  Parsing and document conversions execute inside browser memory. Your documents are never sent online, protecting file values.
                </p>
              </div>
            </div>
          </section>
